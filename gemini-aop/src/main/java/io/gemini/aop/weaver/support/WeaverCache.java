...
public class WeaverCache implements Closeable {
    ...
    private final ConcurrentMap<Class<?>, ConcurrentMap<String /* methodSignature */, Joinpoint.Descriptor>> typeJoinpointDescriptorCache;


    WeaverCache(AopContext aopContext) {
    ...
    public TypeCache getTypeCache(ClassLoader classLoader, String typeName) {
        ...
        if(aopContext.isExcludedClassLoader(ClassLoaderUtils.getClassLoaderName(classLoader)) == true)
    ...
    public Joinpoint.Descriptor getJoinpointDescriptor(Lookup lookup, String methodSignature) {
        ...
        // create fetch from cache
        if(this.typeJoinpointDescriptorCache.containsKey(type) == false) {
            this.typeJoinpointDescriptorCache.computeIfAbsent(
                    type, 
                    key -> {
                        ConcurrentMap<String /* methodSignature */, Joinpoint.Descriptor> joinpointDescriptors = new ConcurrentHashMap<>();
        ...
        ConcurrentMap<String, Joinpoint.Descriptor> joinpointDescriptors = this.typeJoinpointDescriptorCache.get(type);
        return joinpointDescriptors.get(methodSignature);
    }

    protected Joinpoint.Descriptor doCreateJoinpointDescriptor(Lookup lookup, 
            String methodSignature, MethodDescription methodDescription,
            List<? extends Aspect> aspectChain) {
        if(methodDescription.isTypeInitializer()) {
            return new Joinpoint.Descriptor(lookup, methodSignature, null, aspectChain);
        }
        ...
        try {
            ...
            String accessibleName = thisClass.getName() 
                    ...
                    + "(..)";
            return new Joinpoint.Descriptor(lookup, accessibleName, accessibleObject, aspectChain);
        } catch (Exception e) {
