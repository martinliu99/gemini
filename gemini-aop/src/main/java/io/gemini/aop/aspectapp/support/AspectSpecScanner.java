...
public interface AspectSpecScanner<T extends AspectSpec> {
    ...
    List<AspectSpecHolder<T>> scan(AspectContext aspectContext);
    ...
    abstract class AbstractBase<T extends AspectSpec> extends ClassScanner.AccessibleClassInfoFilter
        ...
        @Override
        public List<AspectSpecHolder<T>> scan(AspectContext aspectContext) {
            Assert.notNull(aspectContext, "'aspectContext' must not be null");

            List<AspectSpecHolder<T>> aspectSpecHolders = null;
            try {
                aspectSpecHolders = this.doScanSpecs(aspectContext);
        ...
        protected abstract List<AspectSpecHolder<T>> doScanSpecs(AspectContext aspectContext);
        ...
        public boolean accept(ClassInfo classInfo) {
            ...
            LOGGER.warn("Ignored AspectSpec since AspectSpec class must be public top-level class or public static member class (non-interface, non-abstract, non-enum, non-inner, non-local). \n  {}: {} \n"
                    ...
                    getSpecType(), classInfo.getName(), ClassScanner.Ignored.class.getName());
    ...
    public class ForPojoPointcut extends AbstractBase<AspectSpec.PojoPointcutSpec> {
        ...
        /* @see io.gemini.aop.aspectapp.support.AspectSpecScanner.AbstractBase#getSpecType() 
         */
        @Override
        ...
        @Override
        protected List<AspectSpecHolder<AspectSpec.PojoPointcutSpec>> doScanSpecs(AspectContext aspectContext) {
            List<AspectSpecHolder<AspectSpec.PojoPointcutSpec>> aspectSpecHolders = new ArrayList<>();
            doScanImplementor(aspectContext, aspectSpecHolders);
            doScanFactory(aspectContext, aspectSpecHolders);

            return aspectSpecHolders;
        }

        @SuppressWarnings("unchecked")
        protected void doScanImplementor(AspectContext aspectContext, List<AspectSpecHolder<AspectSpec.PojoPointcutSpec>> aspectSpecHolders) {
            List<String> classNames = aspectContext.getClassScanner()
                    .getClassesImplementing( AspectSpec.PojoPointcutSpec.class.getName() )
                    .filter(this)
                    .getNames();
            ClassLoader classLoader = aspectContext.getAspectClassLoader();
        ...
        protected void doScanFactory(AspectContext aspectContext, List<AspectSpecHolder<AspectSpec.PojoPointcutSpec>> aspectSpecHolders) {
            List<String> classNames = aspectContext.getClassScanner()
    ...
    public class ForExprPointcut extends AbstractBase<AspectSpec.ExprPointcutSpec> {
        ...
        @Override
        protected List<AspectSpecHolder<AspectSpec.ExprPointcutSpec>> doScanSpecs(AspectContext aspectContext) {
            List<AspectSpecHolder<AspectSpec.ExprPointcutSpec>> aspectSpecs = new ArrayList<>();
            doScanImplementor(aspectContext, aspectSpecs);
            doScanFactory(aspectContext, aspectSpecs);

            return aspectSpecs;
        }

        protected void doScanImplementor(AspectContext aspectContext, List<AspectSpecHolder<AspectSpec.ExprPointcutSpec>> aspectSpecHolders) {
            List<String> classNames = aspectContext.getClassScanner()
        ...
        protected void doScanFactory(AspectContext aspectContext, List<AspectSpecHolder<AspectSpec.ExprPointcutSpec>> aspectSpecHolders) {
            List<String> classNames = aspectContext.getClassScanner()
    ...
    public class ForAspectJ extends AbstractBase<AspectSpec.AspectJSpec> {
        ...
        @Override
        protected List<AspectSpecHolder<AspectSpec.AspectJSpec>> doScanSpecs(AspectContext aspectContext) {
