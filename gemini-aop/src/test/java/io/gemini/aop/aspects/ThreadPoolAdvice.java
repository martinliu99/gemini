...
public class ThreadPoolAdvice extends Advice.AbstractBeforeAfter<Void, RuntimeException> {
    ...
    public void before(MutableJoinpoint<Void, RuntimeException> joinpoint) throws Throwable {
        Lookup lookup = joinpoint.getThisLookup();
        MethodHandle workQueueHandle = lookup.findGetter(joinpoint.getThisClass(), "workQueue", BlockingQueue.class);
        ;
        LOGGER.info("before threadpool's execute: " + workQueueHandle.invoke(joinpoint.getThisObject()));
    }
